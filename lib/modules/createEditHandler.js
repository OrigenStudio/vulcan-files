...
const createEditHandler = (attribute, FSCollection) =>
  function editHandler(modifiers, document, currentUser) {
    if (modifiers.$set && modifiers.$set[attribute]) {
      const streamOrId = modifiers.$set[attribute];
      if (streamOrId instanceof Readable) {
        // TODO investigate of the streamOrId deletes the files automatically
        return new Promise((resolve, reject) => {
          // -----------------------------------------------------
          // I am not sure this is required... the streamOrId already contains a path to a file.
              ...
              (error, file) => {
                if (error) {
                  ...
                  resolve({
                    ...
                    filename: streamOrId.filename, // TODO get name from file?
                    path: mapValues(file.versions, (version, versionName) =>
                      FSCollection.link(file, versionName)
                    ),
                  });
                }
              },
