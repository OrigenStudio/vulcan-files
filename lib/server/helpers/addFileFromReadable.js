...
export default async function addFileFromReadable(FSCollection, file) {
  const readable = file.createReadStream();
  // For custom collections that support addFileFromReadable
  if (!!FSCollection.addFileFromReadable) {
    return await FSCollection.addFileFromReadable(readable, file);
    ...
    return await FSCollection.addFileFromBuffer(data, file);
  } else {
