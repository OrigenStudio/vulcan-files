...
export default function createUploadHandlers(options) {
  ...
  if (FSCollection.constructor.name === "FilesCollection") {
    const curryOnCreate = multiple
      ? oldCreateHandlerMultiple
      : oldCreateHandler;
    const curryOnUpdate = multiple
      ? oldUpdateHandlerMultiple
      : oldUpdateHandler;
...
export const oldCreateHandler = (fieldName, FSCollection, getValue) =>
  async function oldCreateHandler({ newDocument, currentUser }) {
    const fieldValue = newDocument[fieldName];
...
export const oldCreateHandlerMultiple = (fieldName, FSCollection, getValue) =>
  async function oldCreateHandlerMultiple({ newDocument, currentUser }) {
    const fieldValue = newDocument[fieldName];
    if (Array.isArray(fieldValue)) {
      return Promise.all(
...
export const oldUpdateHandler = (fieldName, FSCollection, getValue) =>
  async function oldUpdateHandler({ data, document, currentUser }) {
    ...
    if (!data[fieldName]) {
      ...
      return fieldValue.createReadStream
        ? getValue(
            await addFileFromReadable(
              FSCollection,
              fieldValue.createReadStream()
            )
          )
        : fieldValue;
    }
...
export const oldUpdateHandlerMultiple = (fieldName, FSCollection, getValue) =>
  async function oldUpdateHandler({ data, document, currentUser }) {
    if (typeof data[fieldName] === "undefined") return undefined; // no change
    if (!data[fieldName]) {
      // TODO trigger file removal
